...
void *mymalloc(size_t size)
{
  ...
  if (head && lastBlock) /* nth mymalloc call */
  {
    ...
    void *found = findBlock(sizeRounded); // find best fit for allocation
    splitBlock(found, sizeRounded);  /* split block into 2 smaller blocks
                                         if necessary */
    return found;
  }
  else /* 1st mymalloc call */
  {
    head = (block_header *) sbrk(0); // create head
    void *err = sbrk(SIZE_INIT + SIZE_BLOCK_HEADER);
    if (err == (void*)-1) // check if allocation is possible (enough place)
    {
      return NULL;
    }
...
void *findBlock(size_t size)
{
  ...
  int bestSize = SIZE_INIT + 1; // size of closest memory zone to size asked
  ...
  while (!atLimit) // while the end of the heap isn't reached
  {
    ...
    while ( (current + unAlloc_size)<lastBlock
            && (current + unAlloc_size)->alloc == 0
            && unAlloc_size != size) // size of a memory zone unallocated
    {
      unAlloc_size += (current + unAlloc_size)->size;
    }
    if (current == lastBlock  // if the current is the last block...
        || current + unAlloc_size == lastBlock) // ... our the one before
    {
      atLimit = 1;
    }
    ...
    else if ( unAlloc_size > size         // if the size is big enough...
              && unAlloc_size < bestSize) // ... and smaller than bestSize
    {
      goodSize = 1;
    ...
    else if ( unAlloc_size < size      // if the size is wrong but we...
              && !atLimit)             //... can go further in the heap
    {
  ...
  if(goodSize == 0) // no size or bestFit have been given
  {
    ...
    {
      return NULL; // no place to save the data
    }
